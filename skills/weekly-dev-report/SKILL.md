...
- `other`: not rated. Rating cell = `—`, why-line = `Not tracked (role: other).` Excluded from the throughput tables. **Still run the stalled/stuck checks on their tickets** — if any of their sprint tickets haven't moved, surface them in the 🚩 Stuck-tickets section and the 🎯 Delivery-risk At-risk items list with the note `untracked member — issue not moving`.
...
- the **🎯 Delivery risk & recommended actions → At-risk items** list (if they have an active sprint ticket that hasn't moved at all this sprint).

The header should state how many roster members were dropped under this filter so the reader knows the table is filtered, e.g. "_4 roster members omitted (no Jira movement and no authored PRs this week — see the 🎯 At-risk items if applicable)_".
