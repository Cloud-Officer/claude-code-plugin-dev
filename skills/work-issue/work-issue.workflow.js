...
function buildImplementPrompt(issue) {
  ...
  return [
    ...
    '   behaviour-altering change must be covered by tests that pass:',
    '   - Detect the project\'s test runner/framework from the manifest/config; match the nearest existing',
    '     tests\' style. If a language has more than one framework, pick per-directory from the nearest tests',
    '     (never mix two frameworks in one file); if genuinely ambiguous, choose the most common and record it',
    '     in `assumptions[]`.',
    '   - Cover the success path AND the negative/failure paths AND the regression surface of anything shared',
    '     you changed. For a Bug, the regression test must FAIL before the fix and PASS after.',
    ...
    '   - Coverage floor: line >= 80% and branch >= 80% on the changed code, or the repo\'s configured threshold',
    '     if higher (never lower it). Where the language has no branch metric (e.g. Go), meet line/statement',
    '     >= 80% and cover each branch with a table row. Record the actual numbers in `test_status`.',
