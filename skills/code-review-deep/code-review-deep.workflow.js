...
const SHARED_RULES = [
  ...
  '- Tools, packages or first-party tooling installed at their latest/unpinned version (npm/pip/gem installs, "latest"',
  '  runtime inputs, resolving the newest release tag at run time) - a deliberate "always current" policy, not a finding.',
  '- The job-scoped GITHUB_TOKEN (github.token) exposed to package-install or toolchain-download steps to avoid registry',
  '  rate limits - ephemeral, single-repository and bounded by the workflow permissions block. Flag only when a long-lived',
  '  PAT or org secret reaches those steps.',
  '- Unquoted expansion of an input meant to carry several arguments (e.g. ${PARAMETERS}, ${APT_PACKAGES}) - the value is',
  '  written by whoever already controls the workflow, so it is not a trust boundary.',
  '',
  '## Severity levels',
...
const A_QUALITY = {
  ...
  prompt: [
    ...
    'The canonical rule is the co-dev code-standards skill; flag every violation of it:',
    '- overlong narrative comments no human will maintain - any comment block of 3+ lines.',
    '- any comment that justifies a CHANGE rather than explaining the CODE (why this approach was picked, what was',
    '  rejected, how a sibling module does it, what convention is being matched).',
    '- any comment on code that is NOT THERE: annotating a removed line, a deliberate omission, a field left unset,',
    '  or a rule/policy/invariant the code merely complies with. There is nothing at that location to misread, so the',
    '  comment only records that the author thought about it - a test is what protects such an absence. Watch for the',
    '  one-line form of this, which reads as disciplined and is the same defect.',
    '- any comment whose deletion would leave the code no harder to UNDERSTAND (as opposed to no harder to justify).',
    'Test files count: a motivation block above a test case is the same defect, since the test name should carry that.',
    'Flag the block, quote its first line, propose the one-line replacement or deletion, and note that the rationale',
    'belongs in the PR body.',
    'Also flag comments citing an issue/ticket/PR number as justification (that context belongs',
